...
void scsim_comm_genfunc(scsim_communication_data_t *s) {
   ...
   strcpy(s->entityName,  "DUMMY_MHDL_CFG_AES_TEST");
   s->architectureName  = (char *) 0;
   s->designUnitType = 19;
   s->topSimFileName    = (char *) sc_mem_malloc(36);
   strcpy(s->topSimFileName, "DEFAULT.DUMMY_MHDL_CFG_AES_TEST.sim");
   s->timebase = 2;
   s->ignorelowres = 1;
  ...
  s->jitter_enabled = 0;
   ...
   strcpy(s->CreationDate, "Wed Oct  4 22:04:13 2017");
   ...
   s->ElaborationOptions = (char *) sc_mem_malloc(17);
   strcpy(s->ElaborationOptions, "-full64 aes_test");
   s->VhdlOptions = (char *) sc_mem_malloc(158);
   strcpy(s->VhdlOptions, "-mxunielab -uni_make -nc -Xhdlxmropt -uum -vhtop WORK.dummy_mhdl_cfg_AES_TEST -elaboration_options /tmp/vcs_20171005020412_5822/elaboptfile_5822 -picarchive ");
   ...
   s->scsimCompileDirName  = (char *) sc_mem_malloc(49);
   strcpy(s->scsimCompileDirName, "/afs/umbc.edu/users/b/r/brianw5/home/CMPE691/hw3");
   s->MhdlVerilogTopCount  = 0;
   s->MhdlVerilogTopNames  = (char **) 0;
   s->MhdlVhdlTopCount  = 1;
   s->MhdlVhdlTopNames = (char **) sc_mem_malloc(8*1);
   s->MhdlVhdlTopNames[0] = "WORK.dummy_mhdl_cfg_AES_TEST";
   s->unifiedUseMod = 1;
   s->svaBindEnable = 0;
   s->isSlave = 0;
   s->lic_scsi = 0;
   ...
   s->isFsdbSet = 0;
   s->isFsdbOldSet = 0;
   s->ScsWorkDir = (char *) sc_mem_malloc(49);
   strcpy(s->ScsWorkDir, "/afs/umbc.edu/users/b/r/brianw5/home/CMPE691/hw3");
   s->sp_mem_size = 0;
   s->proc_split_num = 0;
